...
- Serious-comparator gate: the MA full-unrank gate passed with 3/3 certified
  rows and 0/3 MA wall-time wins, mean MA/current ratio 3.7953. The
  ...
  0.7815. Mat-Select2 heap-primitive matched the selected log in 3/3 rows, won
  0/3 timing comparisons, and had mean Mat-Select2/linear ratio 10.3626.
  Mat-Select2 soft-selector matched the selected log in 3/3 rows, won 0/3
  timing comparisons, and had mean soft-selector/linear ratio 109.8413. Full FJ
  remained open/not implemented. The soft-heap gate status is
  `selector_probe_validated_no_speed_win`.
