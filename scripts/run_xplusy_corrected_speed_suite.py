#!/usr/bin/env python3
"""Compare residual-corrected full X+Y against adaptive full X+Y unranking."""
...
def run_case(harness, case: dict[str, Any], target_gap: int, skip_audit: bool) -> dict[str, Any]:
    ...
    if corrected_exps is not None and not skip_audit:
        audit = harness.run_measured(
            "xplusy_corrected_interval_audit",
...
def compare_runs(runs: list[dict[str, Any]], n: int) -> dict[str, Any]:
    ...
    corrected = by_name.get("xplusy_corrected_band")
    audit = by_name.get("xplusy_corrected_interval_audit")
    adaptive = by_name.get("xplusy_full_unrank")
    cm = (corrected or {}).get("metrics", {})
    am = (audit or {}).get("metrics", {})
    xm = (adaptive or {}).get("metrics", {})
    comparison: dict[str, Any] = {
        "corrected_certified": bool(am.get("rank_certified", False)) and am.get("certified_count_le", "") == n,
        "adaptive_same_exps": xm.get("exps", None) == cm.get("exps", ""),
    }
...
def summarize_case(case: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "adaptive_reported_seconds": xm.get("seconds", ""),
        ...
        "adaptive_build_seconds": xm.get("build", ""),
        "corrected_count_seconds": cm.get("count_phase", ""),
        "adaptive_count_seconds": xm.get("count_phase", ""),
        "corrected_band_seconds": cm.get("band_phase", ""),
        "adaptive_band_seconds": xm.get("band_phase", ""),
        "adaptive_calls": xm.get("calls", ""),
        "adaptive_rank_gap": xm.get("rank_gap", ""),
        "center_rank_error": int(cm.get("center_count", 0)) - int(case["N"]) if cm.get("center_count", "") != "" else "",
        "corrected_band_count": cm.get("band_count", ""),
        "adaptive_band_count": xm.get("band", ""),
...
def main() -> int:
    ...
    out_dir = args.out_dir or (ROOT / "results" / "local" / f"xplusy_corrected_speed_suite_{utc_stamp()}")

    report: dict[str, Any] = {
        "schema": "smooth.lattice.xplusy_corrected_speed_suite.v1",
