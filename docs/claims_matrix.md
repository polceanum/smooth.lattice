...
| Kaplan/FJ-style Mat-Select2 heap-primitive improves X+Y selection at N=10^12 | Negative but validated: 3233/3233 exhaustive small validation cases passed; first-k suite had 1/1 comparable exact-log row, 0/1 timing wins, and Mat-Select2 heap-primitive/current linear ratio 12.7823; k=6 and k=8 skipped by active-row cap | Artifact: `results/benchmarks/sorted_matrix_matselect2_heap_firstk_1e12/`; not a soft-heap time-bound implementation |
| Soft-heap comparator obligation | Partially advanced, not closed: a soft-sequence-heap semantics probe validates corruption-set/witness-set invariants, and a Mat-Select2 soft-selector bridge validates on small Cartesian sums, but current timing evidence is negative | Probes: `benchmarks/soft_sequence_heap_probe.cpp` and `benchmarks/smooth_xplusy_fj_loh_workbench.cpp`; no soft-heap timing win |
| Serious-comparator gate status | Gate artifact: MA full-unrank gate passed; output-sensitive X+Y/LOH probe executed at N=10^6; full FJ remains open/not implemented; soft-heap gate advanced to `semantic_probe_validated_not_selector_integrated`; PyNormaliz passed 2/2 toy simplex counts, but 3/3 certified-target rationalized simplexes timed out at the 5-second per-count cap | Artifact: `results/benchmarks/best_known_comparator_gate/`; status artifact and external-count negative result, not a broad best-known claim |
