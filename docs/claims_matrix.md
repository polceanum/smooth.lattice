...
| Soft-heap comparator obligation | Partially advanced, not closed: a soft-sequence-heap semantics probe validates corruption-set/witness-set invariants and simultaneous corruption bounds, but is slow and not selector-integrated | Probe: `benchmarks/soft_sequence_heap_probe.cpp`; no soft-heap timing claim |
